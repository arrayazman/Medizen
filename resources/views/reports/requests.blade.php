@extends('layouts.app')
@section('title', 'Daftar Seluruh Permintaan')
@section('page-title', 'Lacak Permintaan')
...
@section('content')
    <style>
        ...
        .border-custom-soft {
            border-color: rgba(24, 135, 84, 0.2) !important;
        }
    ...
    <div class="row mb-3 align-items-center">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <div>
                <h4 class="fw-bold mb-0 text-slate-800" style="font-size: 1.1rem; letter-spacing: -0.5px;">LAPORAN
                    PERMINTAAN</h4>
                <p class="text-muted small mb-0" style="font-size: 0.7rem;">Lacak dan pantau durasi layanan seluruh
                    permintaan radiologi.</p>
            </div>
        </div>
    </div>

    <div class="card card-medizen mb-3 p-3 bg-light-soft border-0 shadow-sm">
        <form method="GET" action="{{ route('reports.requests') }}" class="row g-2 align-items-end">
            <div class="col-md-3">
                <label class="form-label text-muted fw-bold mb-1" style="font-size: 0.6rem; letter-spacing: 0.5px;">CARI
                    ORDER / PASIEN</label>
                <input type="text" name="q" class="form-control form-control-medizen bg-white fw-semibold"
                    placeholder="No Order, RM, Nama..." value="{{ request('q') }}">
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-3">
                <label class="form-label text-muted fw-bold mb-1"
                    style="font-size: 0.6rem; letter-spacing: 0.5px;">STATUS</label>
                <select name="status" class="form-select form-select-medizen bg-white fw-bold text-slate-700">
    ...
    <div class="card card-medizen border-0 shadow-sm mb-3">
        <div class="card-header border-bottom-0 d-flex justify-content-between align-items-center pb-2 pt-3">
            <div class="d-flex align-items-center">
                ...
                DATA PERMINTAAN & STATUS TERKINI
            </div>
            <span class="text-slate-600 border px-2 py-1 rounded-1 fw-bold font-monospace"
                style="font-size: 0.65rem;">{{ $orders->total() }} DATA</span>
        </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-medizen mb-0">
                    <thead>
                        ...
                        <tr class="text-center align-middle">
                            <th style="width: 100px; border-top: 1px solid #f1f5f9;">ORDER</th>
                    ...
                    <tbody>
                        @forelse($orders as $order)
                            <tr>
                                ...
                                {{-- Durasi --}}
                                <td
                                    class="text-center fw-bold @if(($order->duration_req_sample ?? 0) > 30) text-danger @else text-slate-700 @endif">
                                    {{ $order->duration_req_sample ?? 0 }}</td>
                                <td
                                    class="text-center fw-bold @if(($order->duration_sample_result ?? 0) > 120) text-danger @else text-slate-700 @endif">
                                    {{ $order->duration_sample_result ?? 0 }}</td>
                                <td
                                    class="text-center fw-bold @if(($order->duration_total ?? 0) > 180) text-danger @else text-custom @endif">
                                    {{ $order->duration_total ?? 0 }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="10" class="text-center py-5 text-muted">
                                    ...
                                    <div class="fw-bold" style="font-size: 0.75rem;">Tidak ada data permintaan.</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            @if($orders->hasPages())
                <div class="px-3 py-2 bg-light-soft d-flex justify-content-between align-items-center border-top">
                    <div class="text-muted fw-bold" style="font-size: 0.6rem; letter-spacing: 0.5px;">
                        SHOWING {{ $orders->firstItem() ?? 0 }}-{{ $orders->lastItem() ?? 0 }} / {{ $orders->total() }} DATA
                    </div>
                        ...
                        <table class="table table-medizen mb-0">
                            <thead class="text-center bg-light">
                                <tr>
                                    ...
                                    <th style="width: 25%;">ORDER ➔ SAMPEL (P-S)</th>
                                    <th style="width: 25%;">SAMPEL ➔ HASIL (S-H)</th>
                                    <th style="width: 25%;">ORDER ➔ HASIL (P-H)</th>
                                </tr>
                            </thead>
                            <tbody>
