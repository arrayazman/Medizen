...
@section('page-title', 'Bridging Specimen Radiologi (SIMRS)')
...
<div class="card card-medizen rounded-0 border-0 shadow-none">
    <div class="card-header d-flex justify-content-between align-items-center bg-white border-bottom rounded-0">
        <div>
            <h5 class="mb-0 fw-bold text-slate-800" style="font-size: 1.1rem;">Bridging Specimen Radiologi (SIMRS)</h5>
    ...
    <div class="card-body p-0">
        ...
        <div class="collapse show p-1 bg-light-soft border-bottom" id="filterCollapse">
            <form action="{{ route('satusehat.kirim-specimen') }}" method="GET" class="p-2">
                <div class="row g-1">
            ...
            <table class="table table-medizen mb-0" id="table-data">
                <thead>
                    <tr>
                        ...
                        <th class="py-2 small">Procedure & Sampel Time</th>
                        <th class="py-2 small text-center">ServiceRequest</th>
                        <th class="py-2 small text-center">Specimen ID</th>
                    ...
                    @forelse ($orders as $row)
                        <tr>
                            <td class="ps-3 align-middle" id="td-check-{{ md5($row->noorder.'_'.$row->kd_jenis_prw) }}">
                                ...
                                <input class="form-check-input row-check {{ $row->id_specimen ? 'row-check-sent' : 'row-check-pending' }}" type="checkbox" value="{{ json_encode($row) }}">
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                ...
                                <div class="text-muted" style="font-size: 0.6rem;">ACSN: {{ $row->noorder }} | SAMPEL: {{ $row->tgl_sampel }} {{ $row->jam_sampel }}</div>
                            </td>
                            <td class="text-center align-middle">
                                <code id="label-id-{{ md5($row->noorder.'_'.$row->kd_jenis_prw) }}" class="x-small text-slate-500">{{ $row->id_servicerequest ?: '-' }}</code>
                            </td>
                            <td class="text-center align-middle">
                                <code id="label-id-{{ md5($row->noorder.'_'.$row->kd_jenis_prw) }}" class="x-small text-emerald fw-bold">{{ $row->id_specimen ?: '-' }}</code>
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="text-center py-5">
                                <div class="d-flex flex-column align-items-center opacity-50">
                                    ...
                                    <h6 class="fw-bold small text-uppercase">TIDAK ADA DATA SAMPLE RADIOLOGI</h6>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<script>
    ...
    async function kirimData(row, rowHash) {
        ...
        try {
            ...
            const res = await $.post('{{ route("satusehat.kirim-specimen.post") }}', {
                _token: '{{ csrf_token() }}',
                ...row
            });
            ...
            if (res.ok) {
                ...
                updateRowUI(rowHash, res.id_specimen);
            } else {
    ...
    async function processBatch(selected, title) {
        ...
        for(let i = 0; i < selected.length; i++) {
            ...
            appendLog('info', `[${i+1}/${selected.length}] MEMPROSES SPECIMEN: ${row.noorder} ...`);
