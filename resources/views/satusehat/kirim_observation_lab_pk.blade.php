...
@section('page-title', 'Kirim Observation Lab PK SATUSEHAT')
...
<div class="card card-medizen rounded-0 border-0 shadow-none">
    <div class="card-header d-flex justify-content-between align-items-center bg-white border-bottom rounded-0 py-2">
        <div>
            <h5 class="mb-0 fw-bold text-slate-800" style="font-size: 1.1rem;">Observation Lab PK</h5>
            <div class="text-muted small" style="font-size: 0.65rem;">BRIDGING HASIL PEMERIKSAAN LABORATORIUM PK KE SATUSEHAT Observation Resource</div>
        </div>
    ...
    <div class="card-body p-0">
        <div class="collapse show p-1 bg-light-soft border-bottom" id="filterCollapse">
            <form action="{{ route('satusehat.kirim-observation-lab-pk') }}" method="GET" class="p-2">
                <div class="row g-1">
                    ...
                    <div class="col-md-6 col-12 border-start border-white ps-md-3">
                        <label class="x-small fw-bold text-muted mb-1 d-block">PENCARIAN</label>
                        <input type="text" name="keyword" class="form-control form-control-sm rounded-0 border-0 shadow-none" placeholder="RM / Nama / Pemeriksaan..." value="{{ $keyword }}" style="font-size: 0.65rem; height: 32px;">
                    </div>
            ...
            <table class="table table-medizen table-hover mb-0">
                <thead>
                    <tr>
                        ...
                        <th width="20%">PASIEN & ORDER</th>
                        <th width="30%">PEMERIKSAAN & LOINC</th>
                        <th width="15%" class="text-center">HASIL</th>
                        <th width="12%" class="text-center">ID FHIR</th>
                ...
                <tbody>
                    @forelse($orders as $row)
                        @php 
                            ...
                            $rowId = 'row-' . $row->noorder . '-' . str_replace(['/', '.'], '-', $row->kd_jenis_prw);
                        @endphp
                        <tr id="{{ $rowId }}">
                            <td class="text-center align-middle">
                                <input type="checkbox" class="form-check-input check-item" data-row='{{ $payload }}' data-status="{{ $row->id_observation ? 'update' : 'kirim' }}">
                            </td>
                            <td class="align-middle">
                                ...
                                <div class="text-muted" style="font-size: 0.55rem;">RM: {{ $row->no_rkm_medis }} | Order: {{ $row->noorder }}</div>
                            </td>
                            <td class="align-middle">
                                <div class="fw-bold text-emerald-700" style="font-size: 0.65rem;">{{ $row->Pemeriksaan }}</div>
                                <div class="text-muted" style="font-size: 0.55rem;">{{ $row->code }} - {{ $row->display }}</div>
                            </td>
                            <td class="text-center align-middle font-monospace fw-bold" style="font-size: 0.75rem;">{{ $row->nilai }}</td>
                            <td class="text-center align-middle fhir-id-cell">
                                @if($row->id_observation)
                                    <code class="text-emerald fw-bold px-2 py-1" style="font-size: 0.6rem; background: #ecfdf5;">{{ $row->id_observation }}</code>
                                @else
                                    <span class="text-muted x-small italic opacity-50">Pending</span>
                                @endif
                            </td>
                            <td class="text-center align-middle action-cell">
                                <button class="btn btn-emerald btn-xs px-3 rounded-0 fw-bold border-0 shadow-none text-white" onclick="sendObservation(this)" data-row='{{ $payload }}' style="font-size: 0.6rem;">
                                    {{ $row->id_observation ? 'UPDATE' : 'KIRIM' }}
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="text-center py-5 text-muted small">TIDAK ADA DATA HASIL LAB</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
            ...
            <div class="modal-header py-3 px-4 bg-emerald text-white border-0 rounded-0">
                <div class="d-flex align-items-center">
                    <div class="bg-white bg-opacity-20 p-2 rounded-3 me-3">
                        <i data-feather="database" class="text-white" style="width: 20px; height: 20px;"></i>
                    </div>
                    <div>
                        <h6 class="modal-title fw-bold mb-0" style="letter-spacing: 0.5px;">PROSES TRANSMISI SATUSEHAT</h6>
                        <div class="x-small opacity-75">Lab PK Observation Synchronization...</div>
                    </div>
                </div>
...
<script>
    ...
    async function sendObservation(btn) { await doProcess([$(btn).data('row')]); }
    ...
    async function doProcess(rows) {
        ...
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const rowId = 'row-' + row.noorder + '-' + row.kd_jenis_prw.replace(/[\/\.]/g, '-');
            const $targetRow = $('#' + rowId);
            $('#process-stats').text(`Processing ${i+1}/${rows.length}...`);
            appendLog('info', `PROSES: ${row.no_rawat} - ${row.Pemeriksaan}`);
            try {
                const res = await $.post('{{ route("satusehat.kirim-observation-lab-pk.post") }}', { _token: '{{ csrf_token() }}', ...row });
                if (res.ok) {
                    successCount++;
                    appendLog('ok', `SUKSES: ${res.id_observation}`);
                    $targetRow.find('.fhir-id-cell').html(`<code class="text-emerald fw-bold px-2 py-1" style="font-size: 0.6rem; background: #ecfdf5;">${res.id_observation}</code>`);
                    $targetRow.find('.action-cell').html(`<button class="btn btn-emerald btn-xs px-3 rounded-0 fw-bold border-0 shadow-none text-white" onclick="sendObservation(this)" data-row='${JSON.stringify({...row, id_observation: res.id_observation})}' style="font-size: 0.6rem;">UPDATE</button>`);
