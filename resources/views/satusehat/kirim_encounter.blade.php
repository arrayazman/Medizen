...
@section('page-title', 'Bridging Encounter SATUSEHAT (SIMRS)')
...
<div class="card card-medizen rounded-0 border-0 shadow-none">
    <div class="card-header d-flex justify-content-between align-items-center bg-white border-bottom rounded-0">
        <div>
            <h5 class="mb-0 fw-bold text-slate-800" style="font-size: 1.1rem;">Bridging Encounter (SIMRS)</h5>
        ...
        <div class="d-flex gap-2">
            ...
            <button id="btnFinishTerpilih" class="btn btn-dark btn-sm px-3 shadow-none fw-bold rounded-0" style="font-size: 0.7rem;">
                <i data-feather="check-circle" class="me-2" style="width: 14px;"></i> FINISH TERPILIH
            </button>
    ...
    <div class="card-body p-0">
        ...
        <div class="collapse show p-1 bg-light-soft border-bottom" id="filterCollapse">
            <form action="{{ route('satusehat.kirim-encounter') }}" method="GET" class="p-2">
                <div class="row g-1">
            ...
            <table class="table table-medizen mb-0" id="table-data">
                <thead>
                    <tr>
                        ...
                        <th class="py-2 small">Reg Time & Unit/Poli</th>
                ...
                <tbody id="data-list">
                    @forelse ($orders as $row)
                        <tr>
                            <td class="ps-3 align-middle" id="td-check-{{ md5($row->no_rawat) }}">
                                <div class="form-check mb-0">
                                <input class="form-check-input row-check {{ $row->id_encounter ? 'row-check-sent' : 'row-check-pending' }}" type="checkbox" value="{{ json_encode($row) }}" @if(!$row->id_lokasi_satusehat) disabled @endif>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                ...
                                <div class="text-muted" style="font-size: 0.6rem;"><span class="privacy-mask">{{ $row->nm_poli }}</span> | MASUK: {{ $row->tgl_registrasi }} {{ $row->jam_reg }}</div>
                            </td>
                            <td class="text-center align-middle">
                                <span class="badge {{ $row->status_lanjut === 'Ralan' ? 'bg-info' : 'bg-warning' }} x-small mb-1">{{ strtoupper($row->status_lanjut) }}</span>
                            </td>
                            <td class="text-center align-middle">
                                <code id="label-id-{{ md5($row->no_rawat) }}" class="x-small text-emerald fw-bold">{{ $row->id_encounter ?: '-' }}</code>
                            </td>
                            <td class="text-center align-middle" id="td-status-{{ md5($row->no_rawat) }}">
                                @if($row->id_encounter)
                            ...
                            <td class="pe-3 text-end align-middle">
                                @if($row->id_encounter)
                                    <div class="d-flex flex-column gap-1">
                                        <button id="btn-single-{{ md5($row->no_rawat) }}" class="btn btn-info rounded-0 px-2 py-1 x-small fw-bold text-white border-0" 
                                                onclick='kirimData({{ json_encode($row) }}, "{{ md5($row->no_rawat) }}")'
                                                @if(!$row->id_lokasi_satusehat) disabled title="Poli tidak memiliki ID Map Lokasi Satusehat" @endif>
                                            RE-SYNC / UPDATE
                                        </button>
                                        <button id="btn-finish-{{ md5($row->no_rawat) }}" class="btn btn-dark rounded-0 px-2 py-1 x-small fw-bold text-white border-0" 
                                                onclick='finishData({{ json_encode($row) }}, "{{ md5($row->no_rawat) }}")'
                                                @if(!$row->id_lokasi_satusehat) disabled title="Poli tidak memiliki ID Map Lokasi Satusehat" @endif>
                                            FINISH ENCOUNTER
                                        </button>
                                    </div>
                                @else
                                    <button id="btn-single-{{ md5($row->no_rawat) }}" class="btn btn-emerald rounded-0 px-2 py-1 x-small fw-bold text-white border-0" 
                                            onclick='kirimData({{ json_encode($row) }}, "{{ md5($row->no_rawat) }}")'
                                            @if(!$row->id_lokasi_satusehat) disabled title="Poli tidak memiliki ID Map Lokasi Satusehat" @endif>
                                        KIRIM DATA
                                    </button>
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-5">
                                <div class="d-flex flex-column align-items-center opacity-50">
                                    ...
                                    <h6 class="fw-bold small text-uppercase">TIDAK ADA DATA ENCOUNTER</h6>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<script>
    ...
    async function kirimData(row, rowHash) {
        ...
        try {
            ...
            const res = await $.post('{{ route("satusehat.kirim-encounter.post") }}', {
                _token: '{{ csrf_token() }}',
                ...row
            });
            ...
            if (res.ok) {
                ...
                updateRowUI(rowHash, res.id_encounter);
            } else {
    ...
    async function finishData(row, rowHash) {
        const modal = new bootstrap.Modal(document.getElementById('modalLog'));
        $('#log-content').empty();
        modal.show();

        try {
            appendLog('info', 'MENGIRIM PERMINTAAN FINISH ENCOUNTER...');
            const res = await $.post('{{ route("satusehat.kirim-encounter.finish") }}', {
                _token: '{{ csrf_token() }}',
                ...row
            });
            ...
            if (res.ok) {
                appendLog('ok', 'KUNJUNGAN BERHASIL DISUDAHI (FINISHED).');
                $(`#td-status-${rowHash}`).html('<span class="badge-modern bg-dark text-white">FINISHED</span>');
            } else {
                appendLog('err', 'GAGAL FINISH: ' + (res.msg || '').toUpperCase());
            }
        } catch (xhr) {
            appendLog('err', 'FATAL ERROR: ' + xhr.responseText);
        }
    ...
    $('#btnUpdateData').on('click', async function() {
        ...
        processBatch(selected, 'UPDATE DATA TERPILIH', '{{ route("satusehat.kirim-encounter.post") }}');
    });

    // Mass Finish
    $('#btnFinishTerpilih').on('click', async function() {
        const selected = $('.row-check-sent:checked');
        if(selected.length === 0) return Swal.fire('Info', 'Silakan pilih data yang sudah SENT untuk diselesaikan (Finish)!', 'info');
        
        Swal.fire({
            title: 'Konfirmasi Finish',
            text: `Anda akan menyelesaikan (Finish) ${selected.length} kunjungan terpilih. Pastikan diagnosa sudah terisi di SIMRS. Lanjutkan?`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Ya, Finish Sekarang!'
        }).then((result) => {
            if (result.isConfirmed) {
                processBatch(selected, 'FINISH TERPILIH', '{{ route("satusehat.kirim-encounter.finish") }}', true);
            }
        });
    });

    async function processBatch(selected, title, url, isFinish = false) {
        const modal = new bootstrap.Modal(document.getElementById('modalLog'));
        $('#log-content').empty();
        modal.show();
        ...
        for(let i = 0; i < selected.length; i++) {
            ...
            appendLog('info', `[${i+1}/${selected.length}] MEMPROSES ENCOUNTER: ${row.no_rawat} ...`);
            ...
            try {
                ...
                if (res.ok) {
                    appendLog('ok', `BERHASIL: ${row.no_rawat}.`);
                    if (isFinish) {
                        $(`#td-status-${rowHash}`).html('<span class="badge-modern bg-dark text-white">FINISHED</span>');
                    } else {
                        updateRowUI(rowHash, res.id_encounter);
                    ...
                    appendLog('err', `GAGAL (${row.no_rawat}): ` + (res.msg || '').toUpperCase());
                }
            } catch (e) {
                appendLog('err', `FATAL ERROR (${row.no_rawat}): ` + (e.responseText || e.message));
            }
            appendLog('info', '----------------------------------------');
        }
