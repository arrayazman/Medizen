...
@section('page-title', 'Bridging Diagnostic Report Radiologi (SIMRS)')
...
<div class="card card-medizen rounded-0 border-0 shadow-none">
    <div class="card-header d-flex justify-content-between align-items-center bg-white border-bottom rounded-0">
        <div>
            <h5 class="mb-0 fw-bold text-slate-800" style="font-size: 1.1rem;">Bridging Diagnostic Report Radiologi (SIMRS)</h5>
    ...
    <div class="card-body p-0">
        ...
        <div class="collapse show p-1 bg-light-soft border-bottom" id="filterCollapse">
            <form action="{{ route('satusehat.kirim-diagnosticreport') }}" method="GET" class="p-2">
                <div class="row g-1">
            ...
            <table class="table table-medizen mb-0" id="table-data">
                <thead>
                    <tr>
                        ...
                        <th class="py-2 small text-center">ServiceReq/Specimen/Observation</th>
                        <th class="py-2 small text-center">Diagnostic Report</th>
                    ...
                    @forelse ($orders as $row)
                        <tr>
                            <td class="ps-3 align-middle" id="td-check-{{ md5($row->noorder.'_'.$row->kd_jenis_prw) }}">
                                ...
                                <input class="form-check-input row-check {{ $row->id_diagnosticreport ? 'row-check-sent' : 'row-check-pending' }}" type="checkbox" value="{{ json_encode($row) }}">
                                </div>
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                <span class="badge {{ $row->id_servicerequest ? 'bg-emerald' : 'bg-secondary' }} mb-1" style="font-size: 0.55rem;" title="ServiceRequest">SR</span>
                                <span class="badge {{ $row->id_specimen ? 'bg-emerald' : 'bg-secondary' }} mb-1" style="font-size: 0.55rem;" title="Specimen">SP</span>
                                <span class="badge {{ $row->id_observation ? 'bg-emerald' : 'bg-secondary' }} mb-1" style="font-size: 0.55rem;" title="Observation">OB</span>
                                <span class="badge {{ $row->id_imaging ? 'bg-info' : 'bg-secondary' }} mb-1" style="font-size: 0.55rem;" title="ImagingStudy">IMG</span>
                            </td>
                            <td class="text-center align-middle">
                                <code id="label-id-{{ md5($row->noorder.'_'.$row->kd_jenis_prw) }}" class="x-small text-emerald fw-bold">{{ $row->id_diagnosticreport ?: '-' }}</code>
                            </td>
                            <td class="text-center align-middle" id="td-status-{{ md5($row->noorder.'_'.$row->kd_jenis_prw) }}">
                                @if($row->id_diagnosticreport)
                            ...
                            <td class="pe-3 text-end">
                                ...
                                @else
                                    <button id="btn-single-{{ md5($row->noorder.'_'.$row->kd_jenis_prw) }}" class="btn btn-emerald rounded-0 px-2 py-1 x-small fw-bold text-white border-0" 
                                            onclick='kirimData({{ json_encode($row) }}, "{{ md5($row->noorder."_".$row->kd_jenis_prw) }}")'
                                            @if(!$row->id_servicerequest || !$row->id_specimen || !$row->id_observation) disabled title="Lengkapi Resource Sebelumnya" @endif>
                                        KIRIM DATA
                                    </button>
                                @endif
                            </td>
...
<script>
    ...
    async function kirimData(row, rowHash) {
        ...
        try {
            ...
            const res = await $.post('{{ route("satusehat.kirim-diagnosticreport.post") }}', {
                _token: '{{ csrf_token() }}',
                ...row
            });
            ...
            if (res.ok) {
                ...
                updateRowUI(rowHash, res.id_diagnosticreport);
            } else {
    ...
    async function processBatch(selected, title) {
        ...
        for(let i = 0; i < selected.length; i++) {
            ...
            appendLog('info', `[${i+1}/${selected.length}] MEMPROSES DIAGNOSTIC REPORT: ${row.noorder} ...`);
