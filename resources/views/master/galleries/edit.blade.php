...
@section('page-title', 'Pembaruan Konfigurasi Galeri')
    ...
    <div class="medizen-card-minimal">
        <div class="card-header border-bottom bg-slate-50 p-3">
            <h6 class="mb-0 fw-bold text-slate-700 uppercase" style="font-size: 13px; letter-spacing: 1px;">
                EDIT GALLERY SETTING: {{ $gallery->name }}
            </h6>
        </div>

        <form action="{{ route('master.galleries.update', $gallery) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="p-4">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="d-flex gap-4 p-2 bg-slate-50 border rounded mt-1">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="type" id="typePhoto" value="photo" {{ $gallery->type === 'photo' ? 'checked' : '' }}>
                                <label class="form-check-label fw-bold medizen-indicator p-0" for="typePhoto"
                                    style="cursor: pointer;">PHOTO SLIDESHOW</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="type" id="typeVideo" value="video" {{ $gallery->type === 'video' ? 'checked' : '' }}>
            ...
            <div class="p-3 border-top bg-slate-50 d-flex justify-content-end gap-2">
                ...
                <button type="submit" class="btn btn-emerald medizen-btn-minimal">SIMPAN PERUBAHAN</button>
            </div>
        </form>
    </div>
@endsection