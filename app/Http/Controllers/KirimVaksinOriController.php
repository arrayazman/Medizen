...
class KirimVaksinOriController extends Controller
{
    private $ssService;
    ...
    public function index(Request $request)
    {
        ...
        return view('satusehat.kirim_vaksin_ori', compact('orders', 'tgl1', 'tgl2', 'keyword', 'perPage'));
    }
    ...
    public function post(Request $request)
    {
        ...
        try {
            ...
            if (!$request->expire || $request->expire == '0000-00-00') {
                return response()->json([
                    'ok' => false,
                    'msg' => 'Data Tanggal Expire Obat kosong di SIMRS. Silakan lengkapi di Master Obat (Data Barang).',
                    'logs' => [['type' => 'err', 'msg' => 'VALIDATION ERROR: Tanggal Expire tidak ditemukan atau 0000-00-00']]
                ]);
            }

            $expDateInput = date('Y-m-d', strtotime($request->expire));
            if ($expDateInput < $occDateOnly) {
                return response()->json([
                    'ok' => false,
                    'msg' => 'Tanggal Expire Obat (' . $expDateInput . ') lebih lama dari tanggal pemberian (' . $occDateOnly . '). Silakan koreksi data di SIMRS.',
                    'logs' => [['type' => 'err', 'msg' => 'VALIDATION ERROR: EXPIRATIONDATE < OCCURRENCEDATE']]
                ]);
            }

            // Check for Batch/Lot Number
            if (!$request->no_batch || $request->no_batch == '-') {
                return response()->json([
                    'ok' => false,
                    'msg' => 'Nomor Batch / Lot Number kosong. SATUSEHAT mewajibkan nomor batch untuk imunisasi.',
                    'logs' => [['type' => 'err', 'msg' => 'VALIDATION ERROR: LotNumber is missing']]
                ]);
            }
